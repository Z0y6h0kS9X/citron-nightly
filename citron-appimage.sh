...
case "$1" in
    steamdeck)
        ...
        CMAKE_CXX_FLAGS="-march=znver2 -mtune=znver2 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        CMAKE_C_FLAGS="-march=znver2 -mtune=znver2 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
 		PROFILE="steamdeck"
   		EXTERNAL_SDL2="ON"
        TARGET="Steamdeck"
		CCACHE="ccache"
        ...
        ;;
    steamdeck-pgo)
        echo "Making Citron PGO_Optimized Build for Steam Deck"
        # merge PGO data
        cd pgo
        chmod +x ./merge.sh
        ./merge.sh 5 3 1
    ...
    rog)
        ...
        CMAKE_CXX_FLAGS="-march=znver4 -mtune=znver4 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        CMAKE_C_FLAGS="-march=znver4 -mtune=znver4 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
 		PROFILE="steamdeck"
   		EXTERNAL_SDL2="ON"
        TARGET="ROG_ALLY"
		CCACHE="ccache"
        ...
        ;;
    rog-pgo)
        echo "Making Citron PGO Optimized Build for ROG ALLY X"
        # merge PGO data
        cd pgo
        chmod +x ./merge.sh
        ./merge.sh 5 3 1
    ...
    common)
        ...
        CMAKE_CXX_FLAGS="-march=x86-64-v3 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        CMAKE_C_FLAGS="-march=x86-64-v3 -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        TARGET="Common"
		CCACHE="ccache"
		CC="gcc"
		CXX="g++"
        ...
        ;;
    common-pgo)
        echo "Making Citron PGO Optimized Build for Modern CPUs"
        # merge PGO data
        cd pgo
        chmod +x ./merge.sh
        ./merge.sh 5 3 1
        ...
        ;;
    legacy)
        echo "Making Citron Optimized Build for Legacy CPUs"
    ...
    legacy-pgo)
        ...
        ;;
    aarch64)
        echo "Making Citron Optimized Build for AArch64"
        CMAKE_CXX_FLAGS="-march=armv8-a -mtune=generic -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        CMAKE_C_FLAGS="-march=armv8-a -mtune=generic -Ofast -pipe -flto=auto -fuse-ld=mold -w"
        TARGET="Linux"
		CCACHE="ccache"
		CC="gcc"
		CXX="g++"
...
export DEPLOY_QT=1
export OUTNAME="Citron-${COUNT}-${TARGET}-${ARCH}.AppImage"
...
APPBUNDLE="Citron-${COUNT}-${TARGET}-${ARCH}.dwfs.AppBundle"
ln -sfv ./AppDir/eden.svg ./AppDir/.DirIcon.svg
# Use our custom AppStream metadata for the Citron build (safe and reversible)
cp -v ../io.citron.Citron.appdata.xml ./AppDir
./pelf --add-appdir ./AppDir --appbundle-id="Citron-${DATE}-Escary" --compression "-C zstd:level=22 -S26 -B6" --output-to "$APPBUNDLE"
