; Copyright Dolphin Emulator Project / Azahar Emulator Project / Citron Emulator Project / pflyly
...
!define PRODUCT_NAME "Citron Nightly"
!define PRODUCT_DISPLAY_NAME "${PRODUCT_NAME} (${PRODUCT_TOOLCHAIN})"
!define PRODUCT_PUBLISHER "Citron Emulator Developers"
!define PRODUCT_WEB_SITE "https://citron-emu.org/"
...
!define BINARY_SOURCE_DIR "..\citron\build\bin"
!define MUI_ICON "citron.ico"
...
OutFile "Citron-${PRODUCT_VERSION}-Windows-${PRODUCT_TOOLCHAIN}-${PRODUCT_VARIANT}-Installer.exe"
...
Function InstallOptionPageCreate
  !insertmacro MUI_HEADER_TEXT "Installation Options" "Customize your Citron installation"
  ...
  ${NSD_CreateCheckbox} 0u 16u 100% 12u "Enable portable mode (Store Citron user data in install folder)"
...
Function ClickBackupUserData
  ...
  ${If} $BackupUserData == 1
    MessageBox MB_YESNO|MB_ICONQUESTION "Do you want to back up your Citron user data now?" IDYES continue
    StrCpy $BackupUserData 0
    ${NSD_SetState} $BackupUserDataCheckbox $BackupUserData
    Goto end 
...
Function DoBackupUserData
  ...
  StrCpy $R9 "$DOCUMENTS\CitronBackup\$R8"
  CreateDirectory "$R9"

  ; Backup AppData user data
  SetShellVarContext current
  ${If} ${FileExists} "$APPDATA\citron"
    CreateDirectory "$R9\AppData"
    CopyFiles /SILENT "$APPDATA\citron\*" "$R9\AppData\"
  ${EndIf}
  ...
  MessageBox MB_ICONINFORMATION "Your Citron user data has been backed up to:$\n$R9"
FunctionEnd
...
Function InstallOptionPageLeave
  ...
  SetShellVarContext current ; Set current due to citron default config location
  ${If} ${FileExists} "$APPDATA\citron"
    StrCpy $1 1
  ${EndIf}
    ...
    ${If} $0 == 1
      ${If} $1 == 1
        ...
        use_appdata:
          ...
          CopyFiles /SILENT "$APPDATA\citron\*" "$INSTDIR\user\"
...
Function CustomFinishPageCreate
  !insertmacro MUI_HEADER_TEXT "Installation Complete" "Citron has been installed successfully."
  nsDialogs::Create 1018
  ...
  ${NSD_CreateCheckbox} 0u 0u 100% 12u "Launch Citron after install"
  Pop $LaunchCitronCheckbox
  ${NSD_SetState} $LaunchCitronCheckbox $LaunchCitron

  nsDialogs::Show
FunctionEnd

Function CustomFinishPageLeave
  ${NSD_GetState} $LaunchCitronCheckbox $LaunchCitron
  
  ${If} $LaunchCitron == 1
    Exec "$INSTDIR\citron.exe"
  ${EndIf}
FunctionEnd

Function un.CleanUninstallPageCreate
  !insertmacro MUI_HEADER_TEXT "Uninstallation Options" "Customize your Citron uninstallation"
  nsDialogs::Create 1018
...
Function un.CustomFinishPageCreate
  !insertmacro MUI_HEADER_TEXT "Uninstallation Complete" "Citron has been removed from your computer."
  nsDialogs::Create 1018
  ...
  ${NSD_CreateCheckbox} 0u 0u 100% 12u "Get the latest Citron nightly"
...
Function un.CustomFinishPageLeave
  ...
  ${If} $OpenLatest == 1
    ExecShell "open" "https://github.com/Z0y6h0kS9X/citron-nightly/releases/latest"
  ${EndIf}
FunctionEnd
...
Section "Installation"
  ...
  ${If} $CleanInstall == 1
    ${If} $INSTDIR != ""
      ${If} ${FileExists} "$INSTDIR\citron.exe"
        RMDir /r "$INSTDIR"
      ${EndIf}
      ...
      ; Citron always uses current user context for AppData
      SetShellVarContext current
      ${If} ${FileExists} "$APPDATA\citron"
        DeleteRegKey HKCU "Software\citron"
        RMDir /r "$APPDATA\citron"
      ${EndIf}
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_DISPLAY_NAME}\${PRODUCT_DISPLAY_NAME}.lnk" "$INSTDIR\citron.exe"
  ...
  ${If} $DesktopShortcut == 1
    CreateShortCut "$DESKTOP\${PRODUCT_DISPLAY_NAME}.lnk" "$INSTDIR\citron.exe"
  ${EndIf}
...
Section -RegisterUninstallerMetadata
  ...
  WriteRegStr SHCTX "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\citron.exe"
  ...
  WriteRegStr SHCTX "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\citron.exe"
