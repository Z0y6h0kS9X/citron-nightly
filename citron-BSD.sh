...
# Create base pkg dir
PKG_NAME="Citron-${COUNT}-${TARGET}-${ARCH}"
DESTDIR="${PKG_NAME}" ninja install
PKG_DIR="${PKG_NAME}/usr/local"

if [ -f "${PKG_DIR}/bin/citron" ]; then
    CITRON_PATH="${PKG_DIR}/bin/citron"
    
    # Copy all linked libs
    ldd "$CITRON_PATH" | awk '/=>/ {print $3}' | while read -r lib; do
      case "$lib" in
